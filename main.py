...
def input_pet_data():
    ...
    print("\n" + "="*40)
    print("🐾 [메뉴 1] 반려동물 정보 및 활동 입력")
    print("="*40)
    
    # 2차 과제 필수 조건: 문자열, 정수, 실수형 입력 및 형변환
    pet_name = input("▶ 반려동물의 이름을 입력하세요: ")
    pet_age = int(input("▶ 나이를 입력하세요(정수): "))
    pet_weight = float(input("▶ 현재 몸무게를 입력하세요(실수, kg): "))
    
    # 2차 과제 필수 조건: for문을 활용한 리스트 데이터 추가(append)
...
# =============================================================
# [2] 함수 정의 및 분리 2 : 건강 지수 연산 함수
# [3] 매개변수: 활동 데이터 리스트 (지역변수 data_list)
# [4] 반환값: 가중치가 적용된 종합 건강 지수 (float)
# =============================================================
def calculate_health(data_list):
    # 가중치 공식: (산책 * 1.5) + (식사 * 0.8) + (수면 * 1.2)
    # 함수 내부에서만 사용되는 '지역 변수' index에 연산 결과 저장
    index = (data_list[0] * 1.5) + (data_list[1] * 0.8) + (data_list[2] * 1.2)
    
    return index  # [4] 메인 로직으로 연산 결과 반환
...
def print_health_report(score):
    # 예외 처리: 데이터가 아직 입력되지 않았을 경우
    if pet_name == "":
        print("\n⚠ 등록된 반려동물 정보가 없습니다. 1번 메뉴에서 먼저 입력해 주세요!")
        return  # 함수의 실행을 즉시 종료
        
    # 2차 과제 필수 조건: if-elif-else 5단계 등급 판정 로직
    grade = ""
    if score >= 200:
        grade = "S (최상의 컨디션)"
    elif score >= 150:
    ...
    print("\n" + "🐾"*15)
    print(f"▶ [{pet_name}]의 정밀 건강 진단 보고서")
    print(f"▶ 종합 건강 지수 : {score:.2f}점")
    print(f"▶ 최종 컨디션 등급 : {grade}")
    print(f"▶ 특별 칭호 부여 : < {special_title} >")
    print("🐾"*15)
...
while True:
    ...
    if choice == "1":
        ...
        print("\n🐾 프로그램을 종료합니다. 반려동물과 함께 건강하고 행복한 하루 보내세요! 감기 조심하세요!")
        break  # [1] 사용자가 종료를 누를 때만 무한루프 탈출
        
    else:
        print("\n⚠ 올바르지 않은 번호입니다. 1번부터 3번 사이의 숫자를 입력해 주세요.")
